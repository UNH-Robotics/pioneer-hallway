  ...
  private:
    ...
    void applyInitialPose();
		...
		geometry_msgs::PoseWithCovarianceStamped getPose(
						...
						std::vector<amcl_hyp_t>& hyps, int max_weight_hyp);
...
main(int argc, char** argv)
{
  ...
  AmclNode* nodeHandle = new AmclNode();
  amcl_node_ptr.reset(nodeHandle);
...
{
	sensor_msgs::LaserScan* laser_scan = last_laser;
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      geometry_msgs::PoseWithCovarianceStamped p = getPose(laser_scan, hyps, max_weight_hyp);
...
{
	last_laser = new sensor_msgs::LaserScan();
	last_laser->header = laser_scan->header;
	last_laser->angle_min = laser_scan->angle_min;
	last_laser->angle_max = laser_scan->angle_max;
	last_laser->angle_increment = laser_scan->angle_increment;
	last_laser->time_increment = laser_scan->time_increment;
	last_laser->scan_time = laser_scan->scan_time;
	last_laser->range_min = laser_scan->range_min;
	last_laser->range_max = laser_scan->range_max;
	last_laser->ranges = laser_scan->ranges;
	last_laser->intensities = laser_scan->intensities;
}

geometry_msgs::PoseWithCovarianceStamped
AmclNode::getPose(sensor_msgs::LaserScan* laser_scan, 
						std::vector<amcl_hyp_t>& hyps, int max_weight_hyp)
{
